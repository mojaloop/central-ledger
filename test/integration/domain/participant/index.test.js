...
Test('Participant service', async (participantTest) => {
  ...
  await participantTest.test('add participant endpoint', async (assert) => {
    try {
      ...
      await ParticipantEndpointHelper.prepareData(participant.name, 'FSPIOP_CALLBACK_URL_BULK_TRANSFER_PUT', `${testData.endpointBase}/bulkTransfers/{{bulkTransferId}}`)
      await ParticipantEndpointHelper.prepareData(participant.name, 'FSPIOP_CALLBACK_URL_BULK_TRANSFER_ERROR', `${testData.endpointBase}/bulkTransfers/{{bulkTransferId}}/error`)
