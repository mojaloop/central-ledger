...
echo "=> EXPECTED RESULT: Both settlement accounts are now PS_TRANSFERS_RESERVED. Showing 6 records (previously 4). externalReference recorded for payer only."
echo
echo
...
echo "=> EXPECTED RESULT: Settlement state has changed also to PS_TRANSFERS_RESERVED. Showing 3 records (previously 2)."
echo ""
echo
echo
...
echo "=> EXPECTED RESULT: No change here. Showing same 2 records."
echo
echo
...
echo "=> EXPECTED RESULT: 2 records have been inserted - both settlement transfers are RESERVED. Showing total 12 records (previously 10)."
...
ON p.participantId = pc.participantId
ORDER BY 1 DESC"
echo "=> EXPECTED RESULT: NET_SETTLEMENT_RECIPIENT's position has been adjusted for the settlement transfer reservation."
...
ORDER BY 1 DESC"
echo "=> EXPECTED RESULT: Showing history for the participantPosition bottom-up. Now showing 6 records (prevously 4)."
echo
echo
