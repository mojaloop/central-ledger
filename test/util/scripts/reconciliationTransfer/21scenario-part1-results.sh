...
echo "=> EXPECTED RESULT: Transfer duplicate check is performed and a record is inserted. Showing 1 record."
echo
echo

echo "TABLE transfer"
docker exec -it $DB_ID mysql -uroot -e "SELECT * FROM central_ledger.transfer ORDER BY createdDate DESC"
echo "=> EXPECTED RESULT: A transfer record is inserted. Showing 1 record."
echo
echo
...
echo "EXPECTED RESULT: Transfer is fulfilled. Showing 1 record."
echo
echo
...
echo "=> EXPECTED RESULT: Transfer participants are inserted for the prepared reconciliation transfer. Showing 2 records."
echo
echo
...
echo "=> EXPECTED RESULT: Two extensions and externalReference are inserted. Showing 3 records."
echo
echo
...
echo "=> EXPECTED RESULT: Transfer state changed RECEIVED_PREPARE -> RESERVED -> RECEIVED_FULFIL -> COMMITTED. Showing 4 records."
echo
echo
...
ON p.participantId = pc.participantId
ORDER BY 1 DESC"
echo "=> EXPECTED RESULT: dfsp1-SETTLEMENT account balance is -100, Hub-HUB_RECONCILIATION: 100."
echo
echo
...
ORDER BY 1 DESC"
echo "=> EXPECTED RESULT: DR is applied during 2-RESERVED, thus dfsp1-SETTLEMENT account is -100 (10:41)."
echo "CR is applied during 3-COMMITTED, thus Hub-HUB_RECONCILIATION account is 100 (10:30). Showing 2 records."
echo
echo
