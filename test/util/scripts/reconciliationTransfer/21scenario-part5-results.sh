...
echo "=> EXPECTED RESULT: Transfer duplicate check is performed and a record is inserted. Showing 4 records (previously 3)."
echo
echo

echo "TABLE transfer"
docker exec -it $DB_ID mysql -uroot -e "SELECT * FROM central_ledger.transfer ORDER BY createdDate DESC"
echo "=> EXPECTED RESULT: A transfer record for amount of 50 is inserted. Showing 4 records (previously 3)."
echo
echo
...
echo "EXPECTED RESULT: New transfer is not fulfilled yet. Showing same 3 records."
echo
echo
...
echo "=> EXPECTED RESULT: Transfer participants are inserted for the prepared reconciliation transfer. Showing 8 records (previously 6)."
echo
echo
