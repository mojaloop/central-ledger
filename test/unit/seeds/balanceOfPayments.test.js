...
Test('BalanceOfPayments', async (balanceOfPaymentsSeedTest) => {
  ...
  balanceOfPaymentsSeedTest.beforeEach(t => {
    sandbox = Sinon.createSandbox()
    t.end()
  })

  balanceOfPaymentsSeedTest.afterEach(t => {
    sandbox.restore()
    t.end()
  })

  await balanceOfPaymentsSeedTest.test('seed should', async (test) => {
    ...
    try {
      ...
      test.ok(knexStub.withArgs('balanceOfPayments').calledOnce, 'knex called with balanceOfPayments once')
      test.end()
    } catch (err) {
      Logger.error(`balanceOfPayments seed failed with error - ${err}`)
      test.fail()
      test.end()
    }
  })
  ...
  await balanceOfPaymentsSeedTest.end()
})
