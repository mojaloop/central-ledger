...
const Model = require('../../../src/seeds/transactionScenario')

Test('TransactionScenario', async (transactionScenarioTest) => {
  let sandbox

  transactionScenarioTest.beforeEach(t => {
    sandbox = Sinon.createSandbox()
    t.end()
  })

  transactionScenarioTest.afterEach(t => {
    sandbox.restore()
    t.end()
  })
  ...
  await transactionScenarioTest.test('seed should', async (test) => {
    ...
    try {
      ...
      test.ok(knexStub.withArgs('transactionScenario').calledOnce, 'knex called with transactionScenario once')
      test.end()
    } catch (err) {
  ...
  await transactionScenarioTest.end()
})
