...
Test('Endpoint type', async (endpointTypeTest) => {
  ...
  endpointTypeTest.beforeEach(t => {
    sandbox = Sinon.createSandbox()
    t.end()
  })

  endpointTypeTest.afterEach(t => {
    sandbox.restore()
    t.end()
  })

  await endpointTypeTest.test('seed should', async (test) => {
    ...
    try {
      ...
      test.ok(knexStub.withArgs('endpointType').calledOnce, 'knex called with endpointType once')
      test.end()
    } catch (err) {
      Logger.error(`endpointType seed failed with error - ${err}`)
      test.fail()
      test.end()
    }
  })
