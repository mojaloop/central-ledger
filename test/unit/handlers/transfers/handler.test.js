...
Test('Transfer handler', transferHandlerTest => {
  ...
  transferHandlerTest.test('transfer should', transferTest => {
    ...
    transferTest.test('produce a message to the notifications topic - kafka consumer not found', async (test) => {
      await Consumer.createHandler(topicName, config, command)
      Kafka.Consumer.getConsumer.throws(new Error())
      Utility.transformGeneralTopicName.returns(topicName)
      const result = await allTransferHandlers.transfer(null, messages)
    ...
    transferTest.test('produce a message to the notifications topic - autocommit enabled ', async (test) => {
      await Consumer.createHandler(topicName, config, command)
      Kafka.Consumer.isConsumerAutoCommitEnabled.returns(true)
      Utility.transformGeneralTopicName.returns(topicName)
      const result = await allTransferHandlers.transfer(null, messages)
    ...
    transferTest.test('produce a message to the notifications topic on commit - consumer throws error', async (test) => {
      await Consumer.createHandler(topicName, config, command)
      Kafka.Consumer.getConsumer.throws(new Error())
      Utility.transformGeneralTopicName.returns(topicName)
      const result = await allTransferHandlers.transfer(null, commitMessages)
    ...
    transferTest.test('produce a message to the notifications topic on commit - consumer not found', async (test) => {
      await Consumer.createHandler(topicName, config, command)
    ...
    transferTest.test('produce a message to the notifications topic on commit - autocommit enabled', async (test) => {
      await Consumer.createHandler(topicName, config, command)
      Kafka.Consumer.isConsumerAutoCommitEnabled.returns(true)
      Utility.transformGeneralTopicName.returns(topicName)
      const result = await allTransferHandlers.transfer(null, commitMessages)
    ...
    transferTest.test('produce a message to the notifications topic on reject -- consumer not found', async (test) => {
      await Consumer.createHandler(topicName, config, command)
      Kafka.Consumer.getConsumer.returns(null)
      Utility.transformGeneralTopicName.returns(topicName)
      const result = await allTransferHandlers.transfer(null, rejectMessages)
    ...
    transferTest.test('produce a message to the notifications topic on reject -- autocommit is enabled', async (test) => {
      await Consumer.createHandler(topicName, config, command)
      Kafka.Consumer.isConsumerAutoCommitEnabled.returns(true)
      Utility.transformGeneralTopicName.returns(topicName)
      const result = await allTransferHandlers.transfer(null, rejectMessages)
    ...
    transferTest.test('produce a message to the notifications topic on abort', async (test) => {
      await Consumer.createHandler(topicName, config, command)
      Utility.transformGeneralTopicName.returns(topicName)
      const result = await allTransferHandlers.transfer(null, abortMessages)
      test.equal(result, true)
      test.end()
    })

    transferTest.test('produce a message to the notifications topic on abort- consumer not found', async (test) => {
      await Consumer.createHandler(topicName, config, command)
      Kafka.Consumer.getConsumer.returns(null)
      Utility.transformGeneralTopicName.returns(topicName)
      const result = await allTransferHandlers.transfer(null, abortMessages)
    ...
    transferTest.test('produce a message to the notifications topic on abort- autocommit is enabled', async (test) => {
      await Consumer.createHandler(topicName, config, command)
      Kafka.Consumer.isConsumerAutoCommitEnabled.returns(true)
      Utility.transformGeneralTopicName.returns(topicName)
      const result = await allTransferHandlers.transfer(null, abortMessages)
    ...
    transferTest.test('produce a message to the notifications topic on timeout-reserved', async (test) => {
      await Consumer.createHandler(topicName, config, command)
      Utility.transformGeneralTopicName.returns(topicName)
      const result = await allTransferHandlers.transfer(null, timeoutMessages)
      test.equal(result, true)
      test.end()
    })

    transferTest.test('produce a message to the notifications topic on timeout-reserved- consumer not found', async (test) => {
      await Consumer.createHandler(topicName, config, command)
      Kafka.Consumer.getConsumer.returns(null)
      Utility.transformGeneralTopicName.returns(topicName)
      const result = await allTransferHandlers.transfer(null, timeoutMessages)
    ...
    transferTest.test('produce a message to the notifications topic on timeout-reserved - autocimmit is enabled', async (test) => {
      await Consumer.createHandler(topicName, config, command)
      Kafka.Consumer.isConsumerAutoCommitEnabled.returns(true)
      Utility.transformGeneralTopicName.returns(topicName)
      const result = await allTransferHandlers.transfer(null, timeoutMessages)
