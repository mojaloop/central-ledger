  ...
  positionIndexTest.test('processPositionPrepareBin should', changeParticipantPositionTest => {
    ...
    changeParticipantPositionTest.test('produce abort message for when payer does not have enough liquidity', async (test) => {
      ...
      const allSettlementModels = [{
        ...
        autoPositionReset: 1,
        adjustPosition: 0,
        settlementAccountTypeId: 2 // Is this Enum.Accounts.LedgerAccountType.POSITION?
      }]
    ...
    changeParticipantPositionTest.test('produce abort message reserved messages for valid transfer messages', async (test) => {
      const participant = {
        participantId: 0,
        name: 'payerFsp',
        currency: 'USD',
