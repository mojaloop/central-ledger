...
Test('Transfer Service tests', serviceTest => {
  ...
  serviceTest.beforeEach(t => {
    ...
    sandbox.stub(TransferObjectTransform, 'fromRequestToDatabase')
  ...
  serviceTest.test('getById should', getByIdTest => {
    getByIdTest.test('return result from read model', test => {
      ...
      TransferService.getById.withArgs(id).returns(transferPromise)
  ...
  serviceTest.test('getAll should', getByIdTest => {
    getByIdTest.test('return result from read model', test => {
      const transfer = {}
      const transferPromise = P.resolve(transfer)
      TransferService.getAll.returns(transferPromise)
  ...
  serviceTest.test('getFulfilment should', getFulfilmentTest => {
    getFulfilmentTest.test('throw TransferNotFoundError if transfer does not exists', test => {
      const id = Uuid()
      TransferService.getById.withArgs(id).returns(P.resolve(null))
      Service.getFulfilment(id)
        .then(() => {
    ...
    getFulfilmentTest.test('throw TransferNotConditionError if transfer does not have execution_condition', test => {
      const id = Uuid()
      const model = { id }
      TransferService.getById.withArgs(id).returns(P.resolve(model))
    ...
    getFulfilmentTest.test('throw AlreadyRolledBackError if transfer rejected', test => {
      const id = Uuid()
      const transfer = { id, executionCondition: 'condition', state: TransferState.REJECTED }
      TransferService.getById.withArgs(id).returns(P.resolve(transfer))
    ...
    getFulfilmentTest.test('throw MissingFulfilmentError if transfer does not have fulfilment', test => {
      const id = Uuid()
      ...
      Service.getFulfilment(id)
        ...
        .catch(Errors.MissingFulfilmentError, e => {
          test.equal(e.message, 'This transfer has not yet been fulfilled')
        })
        .catch(e => {
          test.fail('Exepected MissingFulfilmentError')
        })
        .then(test.end)
    })

    getFulfilmentTest.test('return transfer fulfilment', test => {
      const id = Uuid()
  ...
  serviceTest.test('prepare should', prepareTest => {
    prepareTest.test('execute prepare function', test => {
      ...
      TransferObjectTransform.fromRequestToDatabase.withArgs(payload).returns(proposedTransfer)
      ...
      TransferObjectTransform.toTransfer.withArgs(preparedTransfer).returns(expectedTransfer)
  ...
  serviceTest.test('fulfil should', fulfillTest => {
    fulfillTest.test('execute fulfil command', function (assert) {
      ...
      TransferObjectTransform.toTransfer.returns({ id: expandedId })
    ...
    fulfillTest.test('Emit transfer executed event', t => {
      ...
      Service.fulfil(payload)
        .then(result => {
          ...
          t.equal(args1.execution_condition_fulfilment, fulfilment)
          t.end()
        })
    })
  ...
  serviceTest.test('reject should', rejectTest => {
    rejectTest.test('execute reject command', test => {
      ...
      TransferObjectTransform.toTransfer.withArgs(transfer).returns(cleanTransfer)
