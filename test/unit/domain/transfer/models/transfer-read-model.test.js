    ...
    getByIdTest.test('find transfer by transferId', test => {
      ...
      TransfersReadModel.getById(id)
        .then(found => {
          ...
          test.ok(joinCreditStub.withArgs('accounts AS ca', 'transfers.payerParticipantId', 'ca.accountId').calledOnce)
          test.ok(joinDebitStub.withArgs('accounts AS da', 'transfers.payeeParticipantId', 'da.accountId').calledOnce)
    ...
    getAllTest.test('return all transfers', test => {
      ...
      TransfersReadModel.getAll()
        .then(found => {
          ...
          test.ok(builderStub.innerJoin.withArgs('accounts AS ca', 'transfers.payerParticipantId', 'ca.accountId').calledOnce)
          test.ok(joinDebitStub.withArgs('accounts AS da', 'transfers.payeeParticipantId', 'da.accountId').calledOnce)
          test.ok(selectStub.withArgs('transfers.*', 'ca.name AS creditAccountName', 'da.name AS debitAccountName').calledOnce)
          test.end()
        })
