...
const processPositionFxFulfilBin = async (
  ...
  if (binItems && binItems.length > 0) {
    for (const binItem of binItems) {
      ...
      Logger.debug(`processPositionFxFulfilBin::fxTransfer:processingMessage: ${JSON.stringify(fxTransfer)}`)
      Logger.debug(`accumulatedFxTransferStates: ${JSON.stringify(accumulatedFxTransferStates)}`)
      Logger.debug(`accumulatedFxTransferStates[commitRequestId]: ${accumulatedFxTransferStates[commitRequestId]}`)
      ...
      if (transferStateId) {
        ...
        Logger.debug(`processPositionFxFulfilBin::fxTransferStateChange: ${JSON.stringify(fxTransferStateChange)}`)

        accumulatedFxTransferStatesCopy[commitRequestId] = transferStateId
        Logger.debug(`processPositionFxFulfilBin::accumulatedTransferStatesCopy:finalizedFxTransferState ${JSON.stringify(transferStateId)}`)
      }
    }
  }
