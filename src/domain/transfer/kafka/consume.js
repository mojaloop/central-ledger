...
const createOnceOffConsumerGroup = (groupId, funcProcessMessage, topics, options) => {
  return new Promise((resolve, reject) => {
      ...
      funcProcessMessage(payload).then(result => {
        if (result) {
          ...
          consumer.commit(
            function (err, result) {
              if (err) {
                Logger.info(`onceOffConsumerGroup::['${topics}'] Committing index error: ${JSON.stringify(err)}`)
                cb() // this marks the completion of the processing by the worker
                return reject(false)
              }
              Logger.info(`onceOffConsumerGroup::['${topics}'] Committing index`)
              cb() // this marks the completion of the processing by the worker
              return resolve(true)
            })
          // cb() // this marks the completion of the processing by the worker
        ...
        Logger.error(`onceOffConsumerGroup::[''] funcProcessMessage(${funcProcessMessage.name}) failed with the following reason: ${reason}`)
        cb()
    ...
    consumer.on('message', function onMessage (message) {
      console.log('onceOffConsumerGroup::%s read msg Topic="%s" Partition=%s Offset=%d', this.client.clientId, message.topic, message.partition, message.offset)
      Logger.info(`onceOffConsumerGroup::['${topics}'] Consumed message: ${JSON.stringify(message)}`)
