...
const addLimitAndInitialPosition = async (participantName, limitAndInitialPositionObj) => {
  try {
    ...
    await KafkaUtil.produceGeneralMessage(Config.KAFKA_CONFIG, Producer, Enum.Events.Event.Type.NOTIFICATION, Enum.Transfers.AdminNotificationActions.LIMIT_ADJUSTMENT, createLimitAdjustmentMessageProtocol(payload), Enum.Events.EventStatus.SUCCESS)
...
const recordFundsInOut = async (payload, params, enums) => {
  try {
    ...
    return await KafkaUtil.produceGeneralMessage(Config.KAFKA_CONFIG, Producer, Enum.Events.Event.Type.ADMIN, Enum.Events.Event.Action.TRANSFER, messageProtocol, Enum.Events.EventStatus.SUCCESS)
  } catch (err) {
    throw ErrorHandler.Factory.reformatFSPIOPError(err)
  }
}
