  ...
  try {
    ...
    if (hasDuplicateId && hasDuplicateHash) {
      ...
      if ([TransferState.COMMITTED, TransferState.ABORTED].includes(transferStateEnum)) {
        ...
        await KafkaUtil.proceed(Config.KAFKA_CONFIG, params, { consumerCommit, producer, fromSwitch }, Producer)
        histTimerEnd({ success: true, fspId: Config.INSTRUMENTATION_METRICS_LABELS.fspId })
        return true
      } else {
        Logger.info(Util.breadcrumb(location, `inProgress1--${actionLetter}2`))
        await KafkaUtil.proceed(Config.KAFKA_CONFIG, params, { consumerCommit }, Producer)
        histTimerEnd({ success: true, fspId: Config.INSTRUMENTATION_METRICS_LABELS.fspId })
        return true
      }
      ...
      await KafkaUtil.proceed(Config.KAFKA_CONFIG, params, { consumerCommit, fspiopError: fspiopError.toApiErrorObject(Config.ERROR_HANDLING), producer, fromSwitch }, Producer)
      throw fspiopError
    } else { // !hasDuplicateId
      ...
      if (validationPassed) {
        ...
        await KafkaUtil.proceed(Config.KAFKA_CONFIG, params, { consumerCommit, producer, toDestination }, Producer)
        histTimerEnd({ success: true, fspId: Config.INSTRUMENTATION_METRICS_LABELS.fspId })
        return true
      } else {
...
const fulfil = async (error, messages) => {
  ...
  try {
    ...
    if (!transfer) {
      ...
      await KafkaUtil.proceed(Config.KAFKA_CONFIG, params, { consumerCommit, fspiopError: fspiopError.toApiErrorObject(Config.ERROR_HANDLING), producer, fromSwitch, toDestination }, Producer)
      throw fspiopError
    }
    ...
    if (hasDuplicateId && hasDuplicateHash) {
      ...
      if (transferStateEnum === TransferState.COMMITTED || transferStateEnum === TransferState.ABORTED) {
        ...
        await KafkaUtil.proceed(Config.KAFKA_CONFIG, params, { consumerCommit, histTimerEnd }, Producer)
        histTimerEnd({ success: true, fspId: Config.INSTRUMENTATION_METRICS_LABELS.fspId })
        return true
      } else {
        ...
        const producer = { functionality: TransferEventType.NOTIFICATION, action: TransferEventAction.COMMIT }
        await KafkaUtil.proceed(Config.KAFKA_CONFIG, params, { consumerCommit, fspiopError, producer, fromSwitch }, Producer)
        histTimerEnd({ success: true, fspId: Config.INSTRUMENTATION_METRICS_LABELS.fspId })
        return true
      }
    } else if (hasDuplicateId && !hasDuplicateHash) {
      ...
      if (type === TransferEventType.FULFIL && [TransferEventAction.COMMIT, TransferEventAction.REJECT, TransferEventAction.ABORT, TransferEventAction.BULK_COMMIT].includes(action)) {
        ...
        if (payload.fulfilment && !Validator.validateFulfilCondition(payload.fulfilment, transfer.condition)) {
          ...
          await KafkaUtil.proceed(Config.KAFKA_CONFIG, params, { consumerCommit, fspiopError: fspiopError.toApiErrorObject(Config.ERROR_HANDLING), producer, toDestination }, Producer)
          throw fspiopError
...
const registerPrepareHandler = async () => {
  try {
    const prepareHandler = {
      command: prepare,
      topicName: KafkaUtil.transformGeneralTopicName(Config.KAFKA_CONFIG.TOPIC_TEMPLATES.GENERAL_TOPIC_TEMPLATE.TEMPLATE, TransferEventType.TRANSFER, TransferEventAction.PREPARE),
      config: KafkaUtil.getKafkaConfig(Config.KAFKA_CONFIG, Enum.Kafka.Config.CONSUMER, TransferEventType.TRANSFER.toUpperCase(), TransferEventAction.PREPARE.toUpperCase())
    }
...
const registerFulfilHandler = async () => {
  try {
    const fulfillHandler = {
      ...
      config: KafkaUtil.getKafkaConfig(Config.KAFKA_CONFIG, Enum.Kafka.Config.CONSUMER, TransferEventType.TRANSFER.toUpperCase(), TransferEventType.FULFIL.toUpperCase())
    }
...
const registerGetTransferHandler = async () => {
  try {
    const getHandler = {
      command: getTransfer,
      topicName: KafkaUtil.transformGeneralTopicName(Config.KAFKA_CONFIG.TOPIC_TEMPLATES.GENERAL_TOPIC_TEMPLATE.TEMPLATE, TransferEventType.TRANSFER, TransferEventType.GET),
      config: KafkaUtil.getKafkaConfig(Config.KAFKA_CONFIG, Enum.Kafka.Config.CONSUMER, TransferEventType.TRANSFER.toUpperCase(), TransferEventType.GET.toUpperCase())
    }
