...
const fulfil = async (error, messages) => {
  if (error) {
    util.rethrowFspiopError(error, 'fulfil')
  }
  let message
...
const processFulfilMessage = async (message, functionality, span) => {
  ...
  if (!transfer) {
    ...
    util.rethrowFspiopError(fspiopError, 'processFulfilMessage')
  }
  ...
  if (validActionsForRouteValidations.includes(action)) {
    ...
    if (
      ...
      util.rethrowFspiopError(apiFSPIOPError, 'processFulfilMessage')
    }
  }
...
const getTransfer = async (error, messages) => {
  ...
  if (error) {
    util.rethrowFspiopError(error, 'getTransfer')
  }
  let message = {}
  if (Array.isArray(messages)) {
    message = messages[0]
  ...
  try {
    ...
    if (isFx) {
      ...
      if (!fxTransfer) {
        ...
        await Kafka.proceed(Config.KAFKA_CONFIG, params, { consumerCommit, fspiopError: fspiopError.toApiErrorObject(Config.ERROR_HANDLING), eventDetail, fromSwitch, hubName: Config.HUB_NAME })
        util.rethrowFspiopError(fspiopError, 'getTransfer')
      }
...
const registerPrepareHandler = async () => {
  try {
    ...
    util.rethrowFspiopError(err, 'registerPrepareHandler')
  }
}
...
const registerFulfilHandler = async () => {
  try {
    ...
    util.rethrowFspiopError(err, 'registerFulfilHandler')
  }
}
...
const registerGetTransferHandler = async () => {
  try {
    ...
    util.rethrowFspiopError(err, 'registerGetTransferHandler')
  }
}
