...
describe('handlers/prepare', () => {
  ...
  it('Ignores non COMMITTED/ABORTED fxTransfer on duplicate request.', async () => {
    ...
    await harness.messageBus.prepare(null, [forex.buildMessagePrepare()])
    await harness.redpandaDrainSmart(1, '2000002')

    switch (harness.config.HANDLERS_TRANSFER_POSITION_FUSE) {
      case "UNFUSE": {
        Snapshot.from(`[
  ...
  it('Duplicate fxTransfers callback when in `RECEIVED_FULFIL_DEPENDENT` state.', async () => {
    ...
    await harness.messageBus.prepare(null, [forex.buildMessagePrepare()])
    await harness.redpandaDrainSmart(1, '2000003')
  ...
  it('Duplicate fxTransfers callback when in `COMMITTED` state.', async () => {
    ...
    // Now send the first message again.
    await harness.messageBus.prepare(null, [forex.buildMessagePrepare()])
    await harness.redpandaDrainSmart(1, '2000004')
  ...
  it('Duplicate fxTransfers callback when in `ABORTED` state.', async () => {
    ...
    // Now send the first message again.
    await harness.messageBus.prepare(null, [forex.buildMessagePrepare()])
    await harness.redpandaDrainSmart(1, '2000005')
  ...
  it('Updates the transfer state on prepare forwarded action.', async () => {
    ...
    await harness.messageBus.prepare(null, [forwardedMsg])
    ...
    Snapshot.from(`[
      {
        "errorInformation": {
          "errorCode": "2001",
          "errorDescription": "Internal server error:ignore"
          "extensionList": {
            "extension": [
              {
                "key": "cause",
  ...
  it('not timeout fxTransfer in RESERVED_FORWARDED internal transfer state', async () => {
    ...
    await harness.messageBus.timeout(futureDate(10, 'm'))
    await harness.redpandaDrainSmart(0, '2000011')
  ...
  it('notifies if fxTransfer is not found', async () => {
    ...
    await harness.messageBus.prepare(null, [forwardedMsg])
    await harness.redpandaDrainSmart(1, '2000014')
  ...
  it(
    ...
    async () => {
      ...
      await harness.redpandaDrainSmart(harness.expect.messagesPayment(), '2000121')
