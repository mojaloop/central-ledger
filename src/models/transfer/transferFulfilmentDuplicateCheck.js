...
const getTransferFulfilmentDuplicateCheck = async (transferId) => {
  ...
  try {
    return transferToUuid(await Db.from('transferFulfilmentDuplicateCheck').findOne(transferToBin({ transferId })))
  } catch (err) {
...
const saveTransferFulfilmentDuplicateCheck = async (transferId, hash) => {
  ...
  try {
    return Db.from('transferFulfilmentDuplicateCheck').insert(transferToBin({ transferId, hash }))
  } catch (err) {
